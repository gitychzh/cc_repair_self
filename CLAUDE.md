...
```
Claude Code → :40001 proxy (仅Anthropic↔OpenAI格式转换)
            → :41001 LiteLLM (glm5.1, 11变体×7keys=77 deployments)
            → :41002 LiteLLM (dsv4p, 11变体×7keys=77 deployments)
            → ModelScope API
```
...
| Docker container names | `glm5.1_uni41001`, `dsv4p_uni41002`, `cc_postgres`, `auth_to_api_40001/40002` |
...
| MAX_TOOL_DESC | 800 | 800-4000 | docker-compose.yml env |
| MAX_SCHEMA_DESC | 300 | 300-1200 | docker-compose.yml env |
...
configs/
  ...
  litellm-dsv4p/config.yaml # 41002 LiteLLM配置（11变体×7keys）
...
| LiteLLM 配置 | `/opt/cc-infra/litellm-dsv4p/config.yaml` | `docker restart dsv4p_uni41002` |
...
**删除资源前必须验证其独立价值。** 曾删除11个变体model ID（如`ZHipuAI/GlM-5.1`）以为是"不支持的混合大小写"，但每个变体有独立的200/id/day额度。删除=删除额度容量。正确流程：观察→测试→验证→决定。