...
## 架构（R33.2 dispatcher + MS-NV interleaving）

```
CC (settings.json ANTHROPIC_BASE_URL=40000)
  → :40000 dispatcher (按 request body 的 model 字段路由)
      ├── opus/未知 → :40005 proxy (PROXY_ROLE=cc, primary)  → MS-NV interleaving
      └── sonnet    → :40001 proxy (PROXY_ROLE=cc, fallback)  → MS-only

:40005  cc-proxy(primary)  → _cc /v1/messages → Anthropic→OpenAI 转换 → MS-NV 12-slot interleaving
...
:40001  cc-proxy(fallback) → _cc /v1/messages → Anthropic→OpenAI 转换 → MS glm5.1 v×k cycling
...
| port assignments | 40000=dispatcher, 40001=cc(fallback), 40002=codex, 40003=openai, 40005=cc(primary), 41001=LiteLLM, 7894=mihomo NV proxy |
...
| MIN_OUTBOUND_INTERVAL_S | 2.0 | 0-5 | **R31.9 出站节流秒数（burst 缓解核心，对 MS 和 NV 都生效）** |
| NUM_VARIANTS_GLM51/DSV4P | 10 | 5-10 | 每 backend variant 数 |
| NV_NUM_KEYS | 5 | 1-5 | NVIDIA API key 数量（NV 无 RPM 限制，5 key 仅用于 cycling resilience） |
| NV_PROXY_URL | host.docker.internal:7894 | — | mihomo ♻️US-NV 专用美国代理端口 |
| MS_NV_TOTAL_SLOTS | 12 | 7-12 | 7 MS + 5 NV = 12 slot 交织总数 |
