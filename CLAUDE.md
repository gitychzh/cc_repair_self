...
```
                    ...
                    │
                    → :41001 LiteLLM ms_uni41001 (glm5.2v1k1~v10k7 = 70 dep) [UNIFIED]
                    → ModelScope API
```
...
- 模型ID格式：`{base_model}{agent_suffix}` → 如 `glm5.2_cc`, `glm5.2_ol`, `glm5.2_oc`, `glm5.2_hm`, `glm5.2_cx`
- 无suffix = 默认`_cc`（向后兼容：`glm5.2` = `glm5.2_cc`）
...
- `_cx` → Responses API格式（/v1/responses），Codex CLI专用，需要格式转换（Responses API → Chat Completions → Responses API）
- 所有agent共享相同的error cycling + variant fallback + LiteLLM fallback：429/500/502 key cycling + timeout cycling + thinking_budget fix retry + LiteLLM connection error fallback (R26)
- 错误格式根据agent类型：_cc返回Anthropic格式错误，_ol/_oc/_hm/_cx返回OpenAI格式错误

**Variant×Key 2D Round-Robin + Variant Fallback (R21→R23) 机制：**
- Proxy 维护 2D轮换 counter：request N → variant_idx=(N//NUM_KEYS)%NUM_VARIANTS, key_idx=N%NUM_KEYS → model `glm5.2v{V}k{K}`
...
- **R26: 所有key连接错误（ConnectionRefused/ConnectionError/SocketTimeout） → 尝试fallback LiteLLM容器（ms_uni41002），完整v×k cycling重试**
- LiteLLM fallback也失败 → 合并所有错误分类返回：全429→rate_limit_error（retry-after=180s）；有500→api_error；有502→api_error；有timeout→502 api_error
...
| frontend model_name (agent-facing) | `glm5.2_cc`, `glm5.2_ol`, `glm5.2_oc`, `glm5.2_hm`, `glm5.2_cx` — R23.1/R24 suffix system; backward compat: `glm5.2`=glm5.2_cc, `claude-opus-4-8`=glm5.2_cc, `codex-mini-latest`=glm5.2_cx |
| LiteLLM model_name (internal, R21) | `glm5.2v1k1`~`glm5.2v10k7` — proxy精确指定variant+key |
| Docker container names | `ms_uni41001`, `ms_uni41002`, `cc_postgres`, `auth_to_api_40001/40002` |
| port assignments | 41001=primary(ms_uni41001), 41002=fallback(ms_uni41002) |
...
- **ModuleNotFoundError → proxy崩溃 → CC ConnectionRefused卡死** — R27教训：handlers.py添加了`from .codex import handle_codex_responses`但codex.py文件不存在→proxy启动崩溃→容器无限重启→CC连不上proxy→ConnectionRefused→fallback全部无效（proxy是所有fallback的执行者，proxy死了fallback不可能生效）。修改import后必须确认被引用的.py文件存在于gateway目录中。
...
| UPSTREAM_TIMEOUT | 60 | 30-120 | R27: Per-key HTTPConnection超时（秒），与PROXY_TIMEOUT分离 |

## 项目文件结构

```
configs/
  docker-compose.yml       # Docker编排（5个容器：cc_postgres, ms_uni41001, ms_uni41002, auth_to_api_40001/40002）
  .env.template             # 环境变量模板
  litellm-glm51/config.yaml       # 41001 LiteLLM配置（10v×7k glm5.2 = 70 dep）
  ...
  proxy/
    ...
    gateway/                      # R23.1 模块化gateway包
      ...
      config.py                   # 配置 + AGENT_SUFFIXES + detect_agent_type() + LiteLLM fallback URLs
      handlers.py                 # 请求调度（CC→_handle_messages, OpenAI→_handle_openai_with_cycling, Codex→_handle_codex_responses）
      upstream.py                 # 共享v×k cycling + variant fallback + LiteLLM fallback + error handling
  ...
  claude/
    settings-opc_uname.json        # → opc_uname本机 ~/.claude/settings.json (API_TIMEOUT_MS=600000 ✅ R22)
    settings-opc2_uname.json       # → opc2_uname远程 ~/.claude/settings.json (API_TIMEOUT_MS=600000 ✅ R24)
...
| 转换代理 | `/opt/cc-infra/proxy/gateway/` (模块包: config.py, handlers.py, upstream.py, error_mapping.py, converters.py, stream.py, codex.py, app.py, logger.py) | rebuild + recreate proxy容器 |
...
# 全量重建（包括容器名变更 glm5.2_uni41001 → ms_uni41001）
...
curl -s -X POST http://127.0.0.1:40001/v1/messages \
  ...
  -H "x-api-key: sk-litellm-local" \
  -H "anthropic-version: 2023-06-01" \
  -d '{"model":"glm5.2_cc","messages":[{"role":"user","content":"test"}],"max_tokens":50}'
