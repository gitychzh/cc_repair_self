...
| NV proxy ports | 7894-7899（mihomo ♻️US-NV-K1~K5, region-divided url-test），NV_PROXY_URL_MAP per-key |
...
- **NV API 必须经美国代理** — 直连 35+ 秒（glm5.1），美国代理 2-5 秒。R36.2: per-key proxy(NV_PROXY_URL_MAP), 5个key各用7894-7899地域分散。
...
scripts/
  ...
  compare_proxies.sh / proxy_health_score.py / auto_tune.sh
```
...
# 7894  ♻️US-NV-K1    — 圣何塞(硅谷) NVIDIA API 专用美国代理 (NV Key 1)
# 7895  ♻️US-NV-K2    — 阿什本(东海岸DC) NVIDIA API (NV Key 2)
# 7896  ♻️US-NV-K3    — general US NVIDIA API (NV Key 3)
# 7897  ♻️US-NV-K4    — US 0.1x NVIDIA API (NV Key 4)
# 7899  ♻️US-NV-K5    — all US NVIDIA API fallback (NV Key 5)
