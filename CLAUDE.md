...
**R33.2 MS-NV 交织规则**（R35.3已禁用NV interleaving on ALL ports — NV glm-5.1 API unavailable, 0% success）：原12 slot round-robin（7 MS + 5 NV），现所有端口改为纯MS模式。NV API对deepseek-v4-pro可能可用，但glm-5.1完全不可用（DNS error + 20s timeout）。40003实测：0 success / 12 NV-ALL-FAIL / 50 NV-ERR。
