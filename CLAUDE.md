...
:40003  openai-proxy        → _ol/_oc/_hm chat/completions → OpenAI passthrough → glm5.1 v×k cycling

→ :41001 LiteLLM ms_uni41001 (glm5.1v1k1~v10k7 = 70 dep) → ModelScope
→ :7894 mihomo ♻️US-NV url-test (5 best US nodes) → NVIDIA integrate API (z-ai/glm-5.1)
```
...
**R33.2 MS-NV 交织规则**（R35.3已禁用NV interleaving on ALL ports — NV glm-5.1 API unavailable, 0% success）：原12 slot round-robin（7 MS + 5 NV），现所有端口改为纯MS模式。NV glm-5.1完全不可用（DNS error + 20s timeout）。40003实测：0 success / 12 NV-ALL-FAIL / 50 NV-ERR。
...
| MIN_OUTBOUND_INTERVAL_S | 1.5 | 0-5 | **R35.8: 全部 proxy 对齐 1.5s（40003 从 2.0→1.5，数据证明 2.0 不减少 ABORT）** |
| NUM_VARIANTS_GLM51 | 10 | 5-10 | glm5.1 variant 数 |
| NV_NUM_KEYS | 0 (all ports) | 0-5 | R35.3: ALL proxy ports NV disabled (glm-5.1 unavailable, 0% success); re-enable only when NV API confirmed working |
| NV_PROXY_URL | host.docker.internal:7894 | — | mihomo ♻️US-NV 专用美国代理端口（mihomo按需启动） |
| MS_NV_TOTAL_SLOTS | N/A (all pure MS) | 7-12 | R35.3: all ports pure MS; NV interleaving disabled everywhere |
| NV_TIMEOUT | 20 | 10-60 | R35.1: NV-specific connection timeout |
