...
def is_quota_exhaustion(error_json):
    ...
    passthrough/codex proxies still used keyword matching, which caused:
    ModelScope RPM burst 429 → keyword match "quota"/"exhausted" → classified
    as quota_exhausted → all_non_quota_429=False → retry-after:180/30 →
    client sees long retry-after → too_long (>60s) → client gives up → STUCK.
    cc-proxy correctly returns False → all_non_quota_429=True → retry-after:5
    → client waits 5s → retries → succeeds. This asymmetry was the root cause
    of OpenClaw freezing while CC never froze.
    """
    return False
