...
R29→R35.5: Added PROXY_ROLE → removed dsv4p backend routing (ModelScope delisted).
...
# R35.5: Only glm5.1 backend (dsv4p delisted from ModelScope).
...
# R35.5: _ol/_oc/_hm now route to glm5.1 backend (dsv4p delisted)
...
# R35.5: All suffixes route to glm5.1 backend (dsv4p delisted from ModelScope).
MODEL_MAP = {
    ...
    # ─── glm5.1 backend (_ol/_oc/_hm OpenAI) ───
    # OpenClaw (_ol) — OpenAI format, backend=glm5.1
        ...
        # Backward compat: old suffix aliases → still routes to glm5.1
    "glm5.1_ol": "glm5.1",
    "glm5.1_oc": "glm5.1",
    "glm5.1_hm": "glm5.1",
...
# All backends route to glm5.1 which supports thinking_budget
...
# ─── Variant×Key 2D round-robin (R21, R35.5: removed dsv4p) ──────────────────
