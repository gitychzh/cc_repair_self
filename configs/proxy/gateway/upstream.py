...
def execute_request(handler, oai_body, mapped_model, request_id, metrics, t_start):
    ...
    Notes:
        - oai_body["model"] is set internally to v×k routing name by this function
        - oai_body["stream"] is NOT modified here — the handler decides force-stream policy
          before calling this function. This module just sends whatever stream setting
          is in oai_body.
        - For CC path: handler sets force_stream_for_nonstream before calling
        - For OpenAI path: handler passes stream as-is (no forcing)
    """
    ...
    # (handler sets this before calling, we just record it)
    ...
    for attempt_idx in range(NUM_KEYS):
        ...
        try:
            ...
            if resp.status >= 400:
                ...
                # Return error info to handler for agent-type-specific formatting
    ...
    # Max extra quota waste = 2 keys per request.
    # Only attempt variant fallback when all errors were 429 (pure rate limit).
    # Mixed errors (500/502/timeout) skip fallback — those are real issues.
