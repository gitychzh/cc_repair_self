...
It handles:
  ...
  - MS-NV interleaving (R34.1/R33.2): alternate between MS and NV upstreams
  - NV direct API call (proxy calls NVIDIA integrate API directly via HTTPS proxy)
...
R34.1: NV direct API — no NV LiteLLM containers. Passthrough-proxy calls NVIDIA API directly.
       Same approach as cc-proxy R33.2.
...
from .config import (
    ...
    NV_TIMEOUT,
    MS_NV_TOTAL_SLOTS,
    _next_variant_key_pair, _next_nv_key,
...
def execute_request(handler, oai_body, mapped_model, request_id, metrics, t_start):
    ...
    R34.1: When NV_ENABLED and model="glm5.1", requests alternate between MS and NV
