...
| opc_uname_r2 | opc_uname | 2026-06-01 | 全链路排查401卡死CC根因(enable_pre_call_checks health check标记全deployment unhealthy→retry被禁→401直达CC); disable enable_pre_call_checks; cooldown_time 30→120(KEY5 401循环); lowest_latency_buffer 0.1→0.3(key均衡从2.33:1改善); rolling_window_size 10→30; timeout 120→180同步; request_timeout 600→300同步; 配置同步到两台机器并重启 |